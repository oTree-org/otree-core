...
from otree.common import get_main_module, GlobalState, signer_unsign, AUTH_COOKIE_NAME, AUTH_COOKIE_VALUE
from otree.currency import json_dumps
from otree.database import NoResultFound, session_scope, dbq
...
class _OTreeAsyncJsonWebsocketConsumer(WebSocketEndpoint):
    ...
    async def on_connect(self, websocket: WebSocket) -> None:
        ...
        if (
            ...
            logger.error('rejected un-authenticated access to websocket path {}'.format(
                self.scope['path']
            ))
...
class WSCreateDemoSession(BaseCreateSession):
    ...
    async def post_receive_json(self, form_data: dict):
        ...
        if not config:
            await self.send_json({'validation_errors': f'Session config "{session_config_name}" does not exist.'})
            return
