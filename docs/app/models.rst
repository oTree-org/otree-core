...
which you can later export to CSV for analysis in Stata, Matlab, Excel, etc.
...
the ordinal position in which a participant was assigned to its match. Starts at 0.
...
The monetary bonus the participant gets paid for this subsession (in cents).
...
You can store anything you'd like in here ``participant_label`` could be the number of the participant's computer workstation, their student ID number,
or their name.

You can automatically pass it as a ``GET`` parameter in the start URL. For example::

    http://mysite/..../Initialize?[other parameters here]&participant_label=42
...
Inside the method, you should import the module(s) containing the views you want to use.
...
Subsession
~~~~~~~~~~
A subsession is generally a randomization between treatments, though it could just have one treatment.
